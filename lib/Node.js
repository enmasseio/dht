...
function Node (name) {
  ...
  this.name     = name;
  this.id       = util.sha1(name);        // SHA-1 hash of the name, as an array with 20 bytes
  this.idString = util.sha1String(name);  // SHA-1 hash of the name, as a string with 40 hex characters
    ...
    function lookup () {
      ...
      contacts
          ...
          .filter(function (contact) {
            return !(contact.idString in consulted);
          })
          .forEach(function (contact) {
            // mark this contact as consulted
            consulted[contact.idString] = contact;
