...
function Node (name, address, port) {
  ...
  // attach RPC listeners to the host
  ...
  this.host.on('FIND_NODE', function (sender, method, params, callback) {
    var contacts = me.onFindNode(params.id);
    callback(null, contacts.map(toJSON)); // serialize contacts
  });
  this.host.on('FIND_VALUE', function (sender, method, params, callback) {
    var result = me.onFindValue(params.key);
    callback(null, {
      value: result.value || null,
      nodes: result.nodes ? result.nodes.map(toJSON) : null // serialize contacts
    });
  });
 ...
 * @private
 */
Node.prototype.onStoreNode = function (contact) {
  var me = this;

  if (!(contact instanceof Contact)) {
    throw new Error('Contact expected');
  }
 ...
 * @return {Array.<Contact>}          The k nodes closest to the requested id
...
Node.prototype.findNode = function (id) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    function lookup () {
      ...
      if (!inProgress) {
        // return a maximum of K contacts
        resolve(contacts.slice(0, constants.K));
      }
    }
...
Node.prototype.storeValue = function (key, value) {
  ...
  return me
      ...
      .map(function (contact) {
        return me.sendStoreValue(contact, key, value)
            .then(function () {
              return contact;
            })
            .catch(function () {
 ...
 * @returns {{value: * | null, nodes: Array.<Contact> | null}}
 *                Returns either:
    ...
    function lookup () {
      ...
      contacts
          ...
          .forEach(function (contact) {
            ...
            // request the closest contacts of this contact
            promises[contact.id] = me.sendFindValue(contact, key)
                .cancellable()
                .timeout(constants.LOOKUP_TIMEOUT)
 ...
 * @param {Contact | {id: string, address: string, port: number}} contact
 * @param {String} method
 * @param {Object} [params]
...
Node.prototype.sendFindNode = function (contact, id) {
  return this.send(contact, 'FIND_NODE', { id: id.toString() })
      .then(function (jsonContacts) {
        // deserialize contacts
        return Promise.resolve(jsonContacts.map(toContact));
      });
};
...
Node.prototype.sendFindValue = function (contact, key) {
  return this.send(contact, 'FIND_VALUE', { key: key.toString() })
      .then(function (result) {
        // deserialize contacts
        return Promise.resolve({
          value: result.value || null,
          nodes: result.nodes ? result.nodes.map(toContact) : null
        });
      });
};
