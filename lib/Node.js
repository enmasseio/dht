 ...
 * @returns {{value: * | null, nodes: Contact[] | null}}
 *                Returns either:
 *                  (a) the value when found or
 *                  (b) the k nodes closest to the searched key
 */
Node.prototype.onFindValue = function onFindValue (key) {
 ...
 *                               when not found.
 */
Node.prototype.findValue = function findValue (key) {
  // convert the key to an Id if needed
  if (!(key instanceof Id)) key = new Id(key);
  ...
  return new Promise(function (resolve, reject) {
    ...
    // start the first lookup based on the locally known nodes
    contacts = contacts.concat(me.onFindNode(key));
    lookup();
  });
};
 ...
 * @param {Contact} contact
 */
Node.prototype.join = function join(contact) {
