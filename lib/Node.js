...
// TODO: when receiving a message from another node (onPing, onFindContact, ...), update the k-bucket for the senders nodeID using onStoreContact
 ...
 */
// TODO: not so nice sending over a byte array as id
Node.prototype.onFindContact = function onFindContact (id) {
  ...
  while ((contacts.length < K) &&
      ((index - offset > 0) || (index + offset < this.buckets.length))) {
    offset++;
...
Node.prototype.findContact = function findContact (id) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    // TODO: when looking up nodes, the local buckets should be adjusted accordingly?

    /**
     * Recursive lookup of contacts:
    ...
    function lookup () {
      ...
      contacts
          ...
          .filter(function (contact) {
            return (!(contact.id.hex in consulted));
          })
          .forEach(function (contact) {
            // mark this contact as consulted
            consulted[contact.id.hex] = contact;
    ...
    // start the first lookup based on the locally known contacts
    // TODO: add the node itself as well!
    contacts = contacts.concat(me.onFindContact(id));
    lookup();
  });
};
