...
describe('Node', function() {
  ...
  describe('onStoreNode', function () {
    ...
    it('should store a node connection in the right bucket', function (done) {
      ...
      var contact2 = new Contact({id: id2});
    ...
    it('should not replace existing, alive contact for new contact when the bucket is full', function (done) {
      ...
      var contact2 = new Contact({id: node2.id});
      ...
      for (var i = 0; i < 50; i++) {
        ...
        var contact = new Contact({id: node.id});
        nodes.push(node);
        contacts.push(contact);
      }

      var allNodes = [node1, node2].concat(nodes);

      var bucket, leastSeen;
      Promise
          .all(allNodes.map(openNode))
          .then(function () {
          ...
          .then(function () {
            // contact2 should not be added, leastSeen should be moved to tail
            assert.deepEqual(bucket[bucket.length - 1], leastSeen);

            // close all nodes
            allNodes.map(closeNode);
  ...
  describe('onFindNode', function () {
    ...
    it('should find the closest k contacts from a node having less than k contacts', function (done) {
      ...
      var contact = new Contact({id: sha1('node2')});
    ...
    it('should find the closest k contacts from a node from a filled bucket', function () {
      ...
      var searchedNode = node1.buckets[theLuckyBucket][10];
      ...
      assert.deepEqual(contacts[0], searchedNode);
    });
    ...
    it('should find the closest k contacts from a node, from a non-filled bucket', function () {
      ...
      var searchedNode = node1.buckets[theLuckyBucket][1];
  ...
  describe('findNode', function () {
    ...
    it('should find the closest k contacts in a network one level deep', function (done) {
      ...
      var contact1 = new Contact({id: node1.id});
      var contact2 = new Contact({id: node2.id});
      var contact3 = new Contact({id: node3.id});
    ...
    it('should find the closest k contacts in a network with one level deep and special outlier bucket', function (done) {
      ...
      var k = 20;
      var allContacts;
      var allNodes = nodes.concat(node0);

      // connect all nodes
      Promise.all(allNodes.map(openNode))
          // create contacts and store node connections in node0
          .then(function () {
            return Promise
          ...
          .then(function (contacts) {
            // node0 should not have stored all contacts (only k per bucket)
            ...
            allContacts.push({id: node0.id, address: null, port: null});
    ...
    it('should find the closest k contacts in a network two levels deep', function (done) {
      ...
      // open all nodes
      Promise.all([node1, node2, node3].map(openNode))
    ...
    it('should find the closest k contacts in a network with a dead node', function (done) {
      ...
      var contact1 = new Contact({id: Id.create('node1')});
      var contact2 = new Contact({id: Id.create('node2')}); // Note: node2 does not exist
    ...
    it('should find the closest k contacts in a network with some dead nodes', function (done) {
      ...
      var node3 = new Node('node3');
      var contact1 = new Contact({id: node1.id});
      var contact2 = new Contact({id: Id.create('node2')}); // dead contact
      var contact3 = new Contact({id: node3.id});
      var contact4 = new Contact({id: Id.create('node4')}); // dead contact

      // open nodes 1 and 3
      Promise.all([node1, node3].map(openNode))
  ...
  describe('storeValue', function () {
    ...
    it ('should store a value in a network containing dead nodes', function (done) {
      ...
      var contact1 = new Contact({id: node1.id});
      var contact2 = new Contact({id: node2.id});
      var contact3 = new Contact({id: Id.create('node3')}); // dead contact
      var contact4 = new Contact({id: node4.id});
      var contact5 = new Contact({id: Id.create('node5')}); // dead contact
      ...
      Promise.all(allNodes.map(openNode))
          ...
          .then(function (contacts) {
            ...
            allContacts = node0.buckets
                ...
                .reduce(function (prev, cur) {
                  return prev.concat(cur);
                }, [{id: node0.id, address: null, port: null}]);
            ...
            withValue.forEach(function (contact, index) {
              // find the node corresponding to this contact
              var node = findNode(contact.id);
              assert.equal(Object.keys(node.values).length, 1);
              assert.deepEqual(node.values[sha1('foo')], 'bar');
            });

            withoutValue.forEach(function (contact) {
              var node = findNode(contact.id);
              assert.equal(Object.keys(node.values).length, 0);
            })
          })
        ...
        node1.open(function () {
          ...
          client.on('message', function (message, remote) {
            ...
            assert.deepEqual(response, {
              id: 123,
              error: null,
              result: [contact.toJSON()] // Note: returns JSON
            });
      ...
      freeport(function (err, port1) {
        freeport(function (err, port2) {
          freeport(function (err, port3) {
            ...
            contact1 = new Contact({id: Id.create('node1'), address: address, port: port1});
            contact2 = new Contact({id: Id.create('node2'), address: address, port: port2});
            contact3 = new Contact({id: Id.create('node3'), address: address, port: port3});
